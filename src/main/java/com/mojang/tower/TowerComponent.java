...
public class TowerComponent extends Canvas implements Runnable, MouseListener, MouseMotionListener {
    ...
    public void run() {
        init();
        double lastTime = (System.nanoTime() / 1000000L) / 1000.0d;
        running = true;
        ...
        while (running) {
            synchronized (this) {
                double now = (System.nanoTime() / 1000000L) / 1000.0d;
                int frameTicks = 0;
    ...
    private void renderGame(Graphics2D g, double alpha) {
        ...
        for (int i = 0; i < island.entities.size(); i++) island.entities.get(i).render(g, alpha);

        if (!titleScreen && !won) {
            if (selectedHouseType >= 0) {
                boolean canPlace = island.canPlaceHouse(
                    xMouse - xCenter * 2, yMouse - yCenter * 2, HouseType.houseTypes[selectedHouseType]);
                if (canPlace) {
                    g.drawImage(HouseType.houseTypes[selectedHouseType].getImage(bitmaps),
                        xMouse / 2 - xCenter - 8,
                        yMouse / 2 - yCenter - 11,
                        null);
                }
    ...
    public void mousePressed(final MouseEvent me) {
        synchronized (this) {
            if (me.getX() >= width * 2 - 40 && me.getY() >= height * 2 - 40 && me.getX() <= width * 2 - 40 + 32
                && me.getY() <= height * 2 - 40 + 32) {
            ...
            if (me.getButton() == 1) {
                ...
                if (selectedHouseType >= 0) {
                    island.placeHouse(
                        me.getX() - xCenter * 2, me.getY() - yCenter * 2, HouseType.houseTypes[selectedHouseType]);
                } else {
                    Entity e =
                        island.getEntityAtMouse(xMouse - xCenter * 2, yMouse - yCenter * 2 + 3, new TargetFilter() {
                            public boolean accepts(Entity e) {
                                return e instanceof House;
                            }
                        });
